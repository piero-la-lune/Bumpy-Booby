...
<tr>
	<td>
		...
		<input type="text" class="input-medium" name="project_id[]" value="'.$v.'" required />
		<input type="hidden" name="project_old_id[]" value="'.$v.'" />
		<label>'.Trad::F_DESCRIPTION.'</label>
		<textarea name="project_description[]" rows="3">'.htmlspecialchars($d).'</textarea>
		<p class="help">'.Trad::F_TIP_DESCRIPTION.'</p>
	</td>
	<td class="td-actions">
		<div class="btn-group-up-down">
			<a href="javascript:;" class="btn a-up"><i class="icon-chevron-up"></i></a>
			<a href="javascript:;" class="btn a-down"><i class="icon-chevron-down"></i></a>
		</div>
		<a href="javascript:;" class="btn btn-danger a-remove-project"><i class="icon-trash icon-white"></i></a>
	</td>
</tr>
';
}
...
function getTrColor($v) {
	return '
<tr>
	<td><input type="text" class="input-small color" value="'.$v.'" name="color_hex[]" required /></td>
	<td><span class="label" style="background-color:'.$v.'">'.Trad::W_EXAMPLE.'</span></td>
	<td class="td-actions">
		<div class="btn-group-up-down">
			<a href="javascript:;" class="btn a-up"><i class="icon-chevron-up"></i></a>
			<a href="javascript:;" class="btn a-down"><i class="icon-chevron-down"></i></a>
		</div>
		<a href="javascript:;" class="btn btn-danger a-remove"><i class="icon-trash icon-white"></i></a>
	</td>
</tr>
	';
}
...
function getTrStatus($k, $v) {
	...
	foreach ($config['colors'] as $co) {
		$colors .= ' <a href="javascript:;" class="square" style="background-color:'.$co.'"></a>';
	}
	$remove = ''; $disabled = 'readonly';
	if ($k != DEFAULT_STATUS) {
		$remove = '<a href="javascript:;" class="btn btn-danger a-remove"><i class="icon-trash icon-white"></i></a>';
		$disabled = '';
	}
...
<tr>
	<td>
		<input type="text" class="input-small" value="'.$k.'" name="status_id[]" '.$disabled.' required />
	</td>
	<td>
		<input type="text" class="input-medium input-left" value="'.$v['name'].'" name="status_name[]" required /><a href="javascript:;" class="btn btn-color btn-right"><span class="square" style="background-color:'.$v['color'].'"></span></a>
		<a href="javascript:;" class="btn btn-small btn-home '.$active.'"><i class="icon-home"></i></a>
		<div class="div-pick-color">
			'.$colors.'
		</div>
		<input type="hidden" value="'.$v['color'].'" name="status_color[]" class="input-color" />
		<input type="hidden" value="'.$dashboard.'" name="status_dashboard[]" class="input-home" />
	</td>
	<td class="td-actions">
		<div class="btn-group-up-down">
			<a href="javascript:;" class="btn a-up"><i class="icon-chevron-up"></i></a>
...
function getTrLabel($k, $v, $c) {
	...
	$colors = '';
	foreach ($config['colors'] as $co) {
		$colors .= ' <a href="javascript:;" class="label" style="background-color:'.$co.'">'.Trad::W_EXAMPLE.'</a>';
	}
	$remove = ''; $disabled = 'readonly';
	if ($k != PRIVATE_LABEL) {
		$remove = '<a href="javascript:;" class="btn btn-danger a-remove"><i class="icon-trash icon-white"></i></a>';
	...
	return '
<tr>
	<td><input type="text" class="input-small" value="'.$k.'" name="label_id[]" '.$disabled.' required /></td>
	<td>
		<input type="text" class="input-small input-left" value="'.$v.'" name="label_name[]" required /><a href="javascript:;" class="btn btn-color btn-right"><span class="square" style="background-color:'.$c.'"></span></a>
		<span class="label" style="background-color:'.$c.'">'.$v.'</span>
		<div class="div-pick-color">
			'.$colors.'
		</div>
		<input type="hidden" value="'.$c.'" name="label_color[]" class="input-color" />
	</td>
	<td class="td-actions">
		<div class="btn-group-up-down">
			<a href="javascript:;" class="btn a-up"><i class="icon-chevron-up"></i></a>
...
function getTrGroup($k, $v) {
	...
	return '
<tr>
	<td><input type="text" class="input-small" value="'.$k.'" name="group_id[]" '.$disabled.' required /></td>
	<td><input type="text" class="input-medium" value="'.$v.'" name="group_name[]" required /></td>
	<td class="td-actions">
		<div class="btn-group-up-down">
			<a href="javascript:;" class="btn a-up"><i class="icon-chevron-up"></i></a>
			<a href="javascript:;" class="btn a-down"><i class="icon-chevron-down"></i></a>
...
foreach ($config['projects'] as $k => $v) {
	...
	foreach ($a as $key => $value) {
		$active = '';
		$int = '0';
		if (in_array($key, $v['can_access'])) { $active = ' active'; $int = '1'; }
		$buttons .= '<button type="button" class="btn'.$active.'">'.$value;
		$buttons .= '<input type="hidden" name="permission_projects_'.$k.'_'.$key.'" value="'.$int.'" />';
		$buttons .= '</button>';
	}
	$t_permissions .= '
<tr>
	<td>
		<p class="title">'.str_replace('%name%', $k, Trad::F_PROJECT_X).'</p>
...
foreach ($config['permissions'] as $k => $v) {
	...
	foreach ($a as $key => $value) {
		...
		$buttons .= '<button type="button" class="btn'.$active.'">'.$value;
		$buttons .= '<input type="hidden" name="permission_'.$k.'_'.$key.'" value="'.$int.'" />';
		$buttons .= '</button>';
	}
	$t_permissions .= '
<tr>
	<td>
		<p class="title">'.Trad::permissions($k, 'title').'</p>
		<p class="help">'.Trad::permissions($k).'</p>
		<p class="buttons">'.$buttons.'</p>
	</td>
</tr>
	';
...
<tr>
	<td>
		<div class="t-display">
			<img src="'.Text::identicon($u['username']).'" class="identicon" />
			<p class="title">'.htmlspecialchars($u['username']).'
				<a href="javascript:;" class="a-user-edit a-icon-hover"><i class="icon-edit"></i></a>
				<br /><small>'.$config['groups'][$u['group']].'</small>
			</p>
		</div>
		<div class="i-display">
			<label>'.Trad::F_USERNAME.'</label>
			<input type="text" class="input-normal" name="user_username[]" value="'.htmlspecialchars($u['username']).'" required />
			<label>'.Trad::F_PASSWORD.'</label>
			<input type="password" class="input-normal" name="user_password[]" />
			<p class="help">'.Trad::F_TIP_PASSWORD.'</p>
			<label>'.Trad::F_EMAIL.'</label>
			<input type="email" class="input-large" name="user_email[]" value="'.htmlspecialchars($u['email']).'" />
			<label>'.Trad::F_NOTIFICATIONS.'</label>
			<select name="user_notifications[]">
				'.Text::options(array(
					'never' => Trad::S_NEVER,
					'me' => Trad::S_ME,
			...
			<select class="input-medium" name="user_group[]">'.$select.'</select>
...
$content = '
	...
	<ul class="ul-nav">
		<li><a href="#t1" class="a-icon-hover"><i class="icon-chevron-right"></i>&nbsp;'.Trad::T_GLOBAL_SETTINGS.'</a></li>
		<li><a href="#t2" class="a-icon-hover"><i class="icon-chevron-right"></i>&nbsp;'.Trad::T_APPEARANCE.'</a></li>
		<li><a href="#t3" class="a-icon-hover"><i class="icon-chevron-right"></i>&nbsp;'.Trad::T_ISSUES.'</a></li>
		<li><a href="#t4" class="a-icon-hover"><i class="icon-chevron-right"></i>&nbsp;'.Trad::T_GROUPS.'</a></li>
		<li><a href="#t5" class="a-icon-hover"><i class="icon-chevron-right"></i>&nbsp;'.Trad::T_USERS.'</a></li>
	</ul>

<form method="post" action="'.Url::parse('settings').'" class="form">
	...
	<div class="box-settings">
		...
		<div class="box-in">
			...
			<input type="url" name="url" id="url" class="input-large" value="'.$config['url'].'" required />
			...
			<input type="email" name="email" id="email" class="input-large" value="'.$config['email'].'">
			<p class="help">'.Trad::F_TIP_EMAIL.'</p>

			<label for="language">'.Trad::F_LANGUAGE.'</label>
			<select class="input-small" id="language" name="language">'.Text::options($languages, $config['language']).'</select>
		</div>
	</div>
	...
	<div class="box-settings">
		...
		<div class="box-in">
			...
			<p style="text-align:center"><a href="javascript:;" class="btn btn-success btn-add-project">'.Trad::F_ADD_PROJECT.'</a></p>
		</div>
	</div>
	...
	<div class="box-settings">
		...
		<div class="box-in box-closed">
			<div class="alert alert-info">'.Trad::F_TIP_ID_STATUS.'</div>
			<table class="table">
				<thead>
					<tr>
						<th>'.Trad::W_ID.'</th>
			...
			<p style="text-align:center"><a href="javascript:;" class="btn btn-success btn-add-status">'.Trad::F_ADD_STATUS.'</a></p>
		</div>
	</div>
	...
	<div class="box-settings">
		...
		<div class="box-in">
			<div class="alert alert-info">'.Trad::F_TIP_ID_LABEL.'</div>
			<table class="table">
				<thead>
					<tr>
						<th>'.Trad::W_ID.'</th>
			...
			<p style="text-align:center"><a href="javascript:;" class="btn btn-success btn-add-label">'.Trad::F_ADD_LABEL.'</a></p>
		</div>
	</div>
	...
	<div class="box-settings">
		...
		<div class="box-in box-closed">
			<div class="alert alert-info">'.Trad::F_TIP_ID_GROUP.'</div>
			<table class="table">
				<thead>
					<tr>
						<th>'.Trad::W_ID.'</th>
			...
			<p style="text-align:center"><a href="javascript:;" class="btn btn-success btn-add-group">'.Trad::F_ADD_GROUP.'</a></p>
		</div>
	</div>
